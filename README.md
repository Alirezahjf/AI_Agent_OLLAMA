...
* **اتصال GitHub به دستیار محلی** (`local_agent`): حالا دستیار می‌تواند به
  GitHub وصل شود، پروژه بسازد و آن را به‌طور کامل مدیریت کند — clone، commit،
  push، pull، branch، merge، ساخت Pull Request / Issue / Release و مدیریت
  GitHub Actions. اتصال با **OAuth (ریدایرکت به صفحهٔ GitHub)** یا **توکن
  دسترسی شخصی (PAT)**، با پشتیبانی از **چند اکانت**. راهنمای کامل در
  [`local_agent/README.md` → بخش اتصال GitHub](local_agent/README.md).
  توکن در فایل جدا ذخیره می‌شود و هرگز در `.git/config` نمی‌نشیند.
* **تکمیل لایهٔ تلگرام شخصی** (`local_agent`): موتور `PersonalTelegram` که
  ناقص بود کامل شد (اتصال، لاگین گام‌به‌گام، ۲FA) و قابلیت‌های جدیدی مثل
  آمار، فوروارد انبوه، استخراج رسانه و مدیریت سشن/دستگاه‌ها اضافه شد.

...
> راهنمای GitHub App، کارگزار امن OAuth/PKCE، permissionها و عملیات پشتیبانی‌شده:
> [`../docs/GITHUB_INTEGRATION_FA.md`](../docs/GITHUB_INTEGRATION_FA.md)
...
/telegram chats [kind] [q]  لیست/جست‌وجوی زندهٔ چت‌ها
/telegram contacts [query]  لیست/جست‌وجوی زندهٔ مخاطبین
/telegram stats             آمار زندهٔ چت، مخاطب و ناخوانده
/telegram resolve TARGET    حل امن نام/شناسه/username/شماره
```
...
| `/telegram chats [kind] [query]` | لیست و جست‌وجوی زندهٔ چت‌ها |
| `/telegram contacts [query]` | لیست و جست‌وجوی زندهٔ مخاطبین |
| `/telegram stats` | آمار زندهٔ تلگرام |
| `/telegram resolve TARGET` | حل امن مقصد تلگرام |
...
قرارداد کامل پیاده‌سازی، دادهٔ زنده، API و خطاها در [گزارش ارتقای تلگرام](../docs/TELEGRAM_UPGRADE_FA.md) ثبت شده است.
...
رابط Web/Desktop یک مرورگر مستقیم تلگرام با انتخاب اکانت، تب چت‌ها و مخاطبین، فیلتر نوع، جست‌وجو، آمار، تازه‌سازی زنده و تاریخچهٔ پیام دارد. قابلیت‌های تکمیلی شامل آمار کلی/هر چت، فهرست ناخوانده‌ها، export زندهٔ JSON/TXT، دانلود گروهی رسانه با فیلتر نوع و ارسال/فوروارد گروهی تأییدشده با سقف ۲۰ مقصد است.

خطاهای تلگرام ساختاریافته‌اند و کدهایی مانند `network`، `timeout`، `flood_wait`، `session_revoked`، `privacy_restricted`، `admin_required`، `write_forbidden`، `peer_invalid` و `target_ambiguous` دارند. Web API علاوه بر `detail` سازگار با نسخه‌های قبل، شیء `error` شامل code/retryable/retry_after می‌دهد. فقط خواندن‌های idempotent در خطای گذرای شبکه/timeout دقیقاً یک بار retry می‌شوند؛ ارسال و عملیات مخرب هرگز خودکار تکرار نمی‌شوند و FloodWait با هدر `Retry-After` گزارش می‌شود.

...
این پروژه به ربات تلگرام وصل نمی‌شود؛ ابزارهای `telegram.*` مستقیماً (با Telethon و سشن مستقل هر اکانت) کار می‌کنند. ابزارهای حساب شامل `list_accounts`، `add_account`، `remove_account`، `switch_account` و `get_me` است. برای چت‌ها `list_chats(kind=private|group|channel|bot|all)` نوع را سمت سرور فیلتر می‌کند و هر نتیجه شناسه عددی و برچسب نوع دارد. عملیات خواندن، تاریخچه، جست‌وجو، پروفایل و دریافت مدیا امن‌اند؛ ارسال، پاسخ، فوروارد و حذف destructive و نیازمند تأیید هستند.
